...
public class ControllerClassNameHandlerMapping
    ...
    /**
     * 不检测org.springframework.web.servlet.mvc包中类.
     */
    private Set excludedPackages = Collections.singleton(
     ...
     *
     * @param excludedPackagesIn String[]
     */
    public void setExcludedPackages(String[] excludedPackagesIn) {
        if (excludedPackagesIn != null) {
            excludedPackages = new HashSet(Arrays.asList(
                        excludedPackagesIn));
        } else {
            excludedPackages = Collections.EMPTY_SET;
        }
    }
     ...
     *
     * @param excludedClassesIn Class[]
     */
    public void setExcludedClasses(Class[] excludedClassesIn) {
        if (excludedClassesIn != null) {
            excludedClasses = new HashSet(Arrays.asList(excludedClassesIn));
        } else {
            excludedClasses = Collections.EMPTY_SET;
        }
    }
    ...
    protected void registerControllers(Class controllerType) {
        String[] beanNames = getApplicationContext()
                                 .getBeanNamesForType(controllerType);
    ...
    protected boolean isEligibleForMapping(String beanName, Class beanClass) {
        if (beanClass == null) {
            if (logger.isDebugEnabled()) {
                logger.debug("Excluding controller bean '" + beanName
    ...
    protected String generatePathMapping(Class beanClass) {
        ...
        if (className.endsWith(CONTROLLER_SUFFIX)) {
            path = className.substring(0,
                    className.indexOf(CONTROLLER_SUFFIX));
        } else {
            path = className;
        }

        pathMapping.append(path.toLowerCase(Locale.CHINA));

        //if (MultiActionController.class.isAssignableFrom(beanClass)) {
        if (PrototypeController.class.isAssignableFrom(beanClass)) {
            pathMapping.append("/*");
        } else {
            pathMapping.append("*");
        }
